...
describe('SwigBrowserClient', () => {
  ...
  test('quotes onramp through the local proxy without an API key', async () => {
    const calls: CapturedRequest[] = [];
    const swig = new SwigBrowserClient({
      network: 'devnet',
      fetch: jsonFetch((request) => {
    ...
    const result = await swig.ramp.onramp.quote({
    ...
    expect(calls).toHaveLength(1);
    expect(calls[0]).toMatchObject({
      url: 'https://app.example/api/swig/ramp/onramp/quote',
      method: 'POST',
      body: {
        organizationMeldConfigurationId: 'config_123',
        environment: 'sandbox',
  ...
  test('reads an onramp session through the local proxy without an API key', async () => {
    const calls: CapturedRequest[] = [];
    const swig = new SwigBrowserClient({
      network: 'devnet',
      fetch: jsonFetch((request) => {
    ...
    const result = await swig.ramp.onramp.getSession({
      sessionId: 'session_123',
      environment: 'sandbox',
    });
    ...
    expect(calls).toHaveLength(1);
    expect(calls[0]).toMatchObject({
      url: 'https://app.example/api/swig/ramp/onramp/session/session_123?environment=sandbox',
      method: 'GET',
      body: undefined,
    });
    expect(calls[0]?.headers.has('authorization')).toBe(false);
