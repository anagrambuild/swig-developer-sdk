...
def test_normalizes_current_numeric_prepared_transaction_kinds() -> None:
    participant_set = normalize_prepared_transaction(
        {"transaction": "participant-set-transaction", "kind": 4}
    )
    x402 = normalize_prepared_transaction(
        {"transaction": "x402-transaction", "kind": 5}
    )

    assert participant_set.kind == "create-participant-set"
    assert x402.kind == "x402-payment"
